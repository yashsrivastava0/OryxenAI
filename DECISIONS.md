...
- **Context:** The normal-user Preact shell had exposed Build Preparation, Code Generator, and Preview alongside the three user-ready agents even though the owner is still developing the later stages. Local development had also overloaded `auth.pipeline_mode = "detached"` to make those harnesses convenient, which detached authentication from the entire product and made it impossible to verify the intended Google-authenticated first-three-agent journey independently.
- **Decision:** The authenticated `/app` product exposes exactly Discovery, Content Architect, and Visual Design Director, ending with a durable approved-direction completion state. Each stage remains explicitly started, revised, and approved through its existing owner-scoped production API; no stage auto-chains. Build Preparation and Code Generator are removed only from the normal-product client, not from their backend implementations or server authorization boundaries. Authentication is attached for the normal product. A separate config-owned `auth.development_harness_mode` controls only the existing Build Preparation fixture and Code Generator development routers/pages, allowing local harnesses to remain detached while production and test configurations fail closed as attached.
- **Rejected alternatives:** Keeping later-stage controls visible but disabled or labelled "coming soon" - rejected because it advertises an incomplete user journey and leaves unused production endpoints in the shipped client. Leaving the whole local product detached to support developer harnesses - rejected because it prevents real browser-session restoration, ownership, logout, and admission behavior from being exercised in the product. Removing or weakening authentication on later-stage production APIs - rejected because the release boundary is a frontend/product decision, not permission to bypass server-side ownership or entitlement enforcement. Replacing the existing Supabase/FastAPI auth foundation - rejected because its security boundary is already implemented and tested; this work needs integration and acceptance, not a second auth system.
- **Consequence:** A normal user signs in, creates or resumes one owned portfolio, and can complete the three implemented creative agents with persisted responses and explicit handoffs. Approved Visual Design Direction is the terminal product state for this release and does not start hidden work. Later-stage developer routes remain independently configurable for ongoing development, while their production APIs retain existing bearer-authenticated authorization. Any future product exposure of Build Preparation, Code Generator, or Preview requires a separate release decision and acceptance pass rather than silently expanding this boundary.

## D-062 - Build Preparation emits two Markdown briefs instead of a versioned resource pack (supersedes D-009, D-011, D-013, D-018, D-021, D-025, D-028, D-035, D-051, D-060, D-061; partially revises D-029, D-030, D-031, D-032, D-033)

- **Date & Time:** 2026-09-03 18:30 +05:30 - Claude Code (Sonnet 5 / Anthropic)
- **Status:** decided-implemented (Build Preparation side); decided-not-yet-implemented (Code Generator session-bound ingestion of the new contract)
- **Context:** Build Preparation had become the end product of five prior architectural pivots specifically about where resource bytes live and who fetches them (D-009 -> D-013 -> D-018 -> D-028/29/30 -> D-035 -> D-060/61), accumulating a 5-model-call pipeline (`compose_resource_queries`, `select_resources`, a post-selection component-fetch loop, `write_build_context`, an optional `integrate_cross_route`, an advisory `review_handoff_quality`) plus ~6,000 lines of deterministic pack machinery (`quality.py` 1342 lines, `materializer.py` 1775, `providers.py` 1115, `execution.py` 608, `contracts.py` 727, `packager.py` 507) that downloaded and pixel-inspected real images via PIL, fetched real component source from four registries, built a 9-file JSON pack, ZIPed it, uploaded it to R2, and verified the bytes three separate times with SHA-256 chains. Code Generator's admission layer then re-verified all of that (hash-chain match, exact 9-path membership, 6-way `resolution_type` enforcement) before its own planner ever saw the content -- and that planner (`development_planner.py:34-70`) only ever read 6 of the 9 files anyway. The owner explicitly directed a drastic simplification: no download/pixel-verification logic in Build Preparation; real candidate links/references for images, fonts, and components found via research, never invented; component suggestions, never fetched component source; Markdown output Code Generator can read directly; nothing that limits or hardcodes what Code Generator itself decides at generation time.
- **Decision:** Build Preparation's pipeline is now: (1) the unchanged pure Stage 0 scope compiler (`compiler.py`, `visual_input.py`'s presentation-fallback layer, `input_integrator.py` -- none of this touched bytes and none of it needed to change); (2) deterministic, discovery-only provider research (`resource_research.py` + a trimmed `providers.py` that keeps every search-only function -- Pexels, Pixabay, Fontsource, real UI-component registry discovery -- and deletes every byte-fetching function, including `download_image`, `download_font`, `download_pexels`, and component source fetch); (3) at most one bounded model call, `compose_visual_brief`, whose structured output may only pick a candidate by index from the exact list it was given for that role, or `null` -- picking an index it was not given is a hard validation failure (`validate_visual_brief_output`), which is the entire "never invent a resource" guarantee, enforced structurally rather than by trusting the model; (4) deterministic Markdown assembly (`brief_assembly.py`) -- `content-and-narrative-brief.md` is built entirely from Content Architect's approved copy inserted verbatim by Python (a model never touches it), `visual-and-build-brief.md` combines the model's prose with deterministic resource/component reference tables; each file carries exactly one fenced JSON index block near the top as its only structured data. Both briefs, their content hashes, and the compiled scope are persisted directly on `portfolio_sessions.current_state["build_preparation"]` (the same JSONB-on-session-state pattern Discovery/Content Architect/Visual Design Director already use) -- no ZIP, no object storage, no pack version, no execution-contract slot taxonomy, no checkpoint system (a single model call is cheap enough to simply replay on retry). A local debug mirror writes both `.md` files to disk for developer inspection; it is a courtesy copy, never the source of truth. `BuildPreparationConfig` drops every field that only existed to bound the deleted machinery (`bundle_ttl_days`, `max_bundle_bytes`, `minimum_reuse_hours`, `fixture_upload`, `integration_route_threshold`, `image_source_attempt_maximum`, `component_source_attempt_maximum`, `provider_max_wait_seconds`, `require_live_visual_resources`, the whole `delegated_*` v4 policy block); `[artifact_storage]` (R2) config is untouched since Code Generator's own stable-preview promotion still uses it independently. `materializer.py`, `packager.py`, `quality.py`, `execution.py`, `contracts.py`, and `checkpoint.py` are deleted outright.
- **Consequence (Code Generator side, explicitly not done in this pass):** Code Generator's `development_input.py` no longer imports `build_preparation.contracts`/`packager` (both deleted); its ZIP-content admission (`_validate_admitted_data`, `_validate_projections`, `_validate_execution_contract`) is stubbed to fail closed with `PACK_INGESTION_NOT_MIGRATED` rather than validating a contract that no longer exists upstream -- the ZIP-safety/upload/fixture-tree machinery around it (`_validate_zip`, `_store_source`, `from_upload`, `from_fixture`) is untouched and still exercised by its own tests. `CodeGeneratorService.start()` (session-bound production path) fails closed with `CODE_GENERATOR_INGESTION_NOT_MIGRATED` immediately after confirming Build Preparation is READY, rather than dereferencing `BuildPreparationState.package`/`.handoff_report`, which no longer exist; `_session_source_is_current` (`jobs/handlers/code_generator_verification.py`) now always returns `False` -- any run still bound to the old artifact-based `CodeGeneratorSourceRef` is permanently stale. Genuinely migrating Code Generator's ingestion (`development_input.py`, `session_schemas.py::CodeGeneratorSourceRef`, `core/development_planner.py::build_planner_context()`, `core/resource_scout.py`/`resource_adapters.py`'s 6-way resolution-type handling, the standalone dev-harness fixture/upload flow and its frontend, and the associated test fixtures) to read the new two-brief contract is tracked as explicit, separate follow-up work -- not silently assumed done by this entry. Full mypy/ruff pass clean across `src/` after this change; the Build Preparation unit/API test suites were rewritten for the new contract and pass; the pre-existing full unit+API suite passes unchanged elsewhere.
- **Rejected alternatives:** Keeping the pack-shaped output but just shrinking it (dropping only the byte-embedding, keeping ZIP/JSON/execution-contract structure) -- rejected because the ceremony (hash chains, 9-file admission, checkpointing) was overwhelmingly sized for protecting embedded bytes that D-060 had already stopped embedding, per the owner's explicit "remove complexity" direction. A fully unstructured, prose-only Markdown output with no fenced index block at all -- rejected because Code Generator's downstream deterministic modules (route/resource identity, dedup) need stable IDs, not prose; one small fenced JSON block per file preserves "output is only Markdown files" literally while keeping that a mechanical extraction rather than requiring an LLM to re-derive structure from prose. A full simultaneous Code Generator ingestion rewrite in the same pass -- rejected as its own comparably large project (the admission layer alone is ~900 lines of interdependent hash/shape validation touching a dozen test files, the standalone dev harness, and its frontend); attempting it without dedicated focus risked exactly the kind of subtle path/shape bugs the D-060 rollout's own bugfix trail (`8c7e994` through `c461b49`, all same-day) already demonstrated happen whenever this exact boundary changes.
...
- **Status:** decided-implemented
- **Context:** A 10-run live campaign (Codex/Luna, `efca5de`) found the historical repeated-split/motion-mismatch blocker did not recur, but all 10 runs still failed on other issues, and the pack itself was confirmed genuinely bloated (a 129KB file that is a byte-for-byte duplicate of a sibling field, an 89-entry array written 3 times, a resource array written in full 4 times). Tracing `development_planner.py:34-70` showed the planner already reads only 6 narrow, schema-validated files and never touches the bloated ones, so the bloat is real but not the cause of the 10 failures. Separately, Code Generator already has a complete, live, tested resource-acquisition subsystem (`resource_adapters.py`, shared provider clients) that today fires only for explicit Build Preparation gaps (D-018), while everything Build Preparation resolves is bulk-copied with zero network activity on Code Generator's side.
- **Decision:** Add `deferred_materialized` as a sixth `ResolvedResource.resolution_type` (alongside `local_materialized`, `target_package_binding`, `local_recipe`, `delegated_acquisition`, `execution_gap`). Build Preparation keeps its full existing decision pipeline unchanged (search, rank, license/policy check, real download-and-pixel/text-inspect-and-dedupe verification) but, for image/font/component candidates, stops persisting the verified bytes into its own output pack; it instead emits the same already-decided fields (provider, provider_asset_id/source_reference, license, the deterministic intended local path) it would have emitted for `local_materialized`. Code Generator's existing acquisition adapters — unchanged, already live and tested for gap-slots — fetch that one already-decided resource's bytes directly (no `search()`/selection call) at generation time, into the exact path the plan already references.
- **Rejected alternatives:** Reusing the existing `delegated_acquisition` type for this case - rejected because its policy semantics (`selection: closed_set_only`, a provider-allowlist, a candidate cap) mean genuine multi-candidate search-and-select on Code Generator's side, not a zero-ambiguity fetch of one already-pinned candidate; overloading it would blur two different guarantees and its own tests. A full architectural inversion where Code Generator performs its own independent image/component search and selection - rejected because D-018 specifically rejected "arbitrary web/URL fetches by Code Generator" to guarantee it never has to invent or guess a resource decision, and nothing in the 10-run evidence showed Code Generator's planner was ever exposed to the bloat in the first place, so moving the decision itself downstream would both re-open that guarantee and fix nothing the evidence actually showed was broken. A HEAD-request-only existence check instead of a full verify-then-discard fetch - rejected because Build Preparation's existing pixel/dimension/perceptual-hash dedupe and text-based component policy checks require the actual bytes/source text, and a HEAD request cannot reproduce them.
- **Consequence:** D-018's core guarantee is preserved in full - Build Preparation still makes every resource decision, and Code Generator never selects or invents one. Only *when* bytes are fetched moves later, to generation time, using infrastructure that already existed and was already live for gap-slots. This is expected to shrink pack size (no embedded image/font/component bytes and their sidecar metadata) and ease the recurring pack-expiry friction that has repeatedly blocked live-testing, since resource freshness now depends on the provider at generation time rather than on the pack surviving unexpired. A pinned reference that fails to fetch at generation time must surface a specific diagnostic, never silently substitute a different resource - the same "no stock substitution for evidence" concern D-018 raised.
...
- **Decision:** In `_attempt_repair` (`code_generator_verification.py`), a `FinalRepairError` or `SourceValidationError` from `FinalRepairer.repair()` (an honest cannot-complete/context-mismatch result, or a response that failed host-side content validation such as duplicate paths) is caught separately from other exceptions. It does not return `False` immediately; it re-checks `RepairBudget.can_attempt` and, while the budget still allows another round, calls the repair model again (which now sees a `bounded-simplification` strategy hint via the same fingerprint-recurrence logic already in `repair_policy.py`). Only once the budget itself is exhausted does the function give up and report terminal failure. Any other exception type (genuine infrastructure failures) still aborts immediately, unchanged. `projection.repair_rounds` is updated to `budget.total_used` on every consumed round, not only a successful one, so a run that exhausts its budget on repeated rejections still reports the true attempt count instead of 0.
...
- **Decision:** Production Code Generator is an explicit, idempotent session stage over the same durable core. Start binds the eligible Build Preparation run/scope/object key/ETag/size/SHA/expiry, performs fixed no-context provider and local-toolchain preflight, then downloads and re-verifies the artifact in the worker. Structured calls compare two grounded concepts, compile an `ExperienceBlueprintV2`, generate disjoint host-owned route batches, and perform a configured finite sequence of owner-scoped integration polish rounds (three by default). Every polish response must pass the full source/type audit before it can become a durable checkpoint; a rejected response is charged to the finite repair budget and fed back only through bounded owner context. Known pack slots remain authoritative; only emergent gaps use mediated acquisition. Promotion requires non-stale input, final source/build closure, all-route mobile/tablet/desktop and reduced-motion browser journeys, configurable geometry checks, and atomic replacement of the session-stable preview.
