...
- **Consequence:** `docs/portfolio-production-compiler-proposal.md` is marked superseded but kept for historical record. Implementation (new agent code, retiring `build_preparation/`, rewiring jobs/API/DB, tests) is a separate, not-yet-started planning pass — this entry covers the architecture decision only. Code Generator remains untouched and deferred.
