...
- **Status:** decided-implemented
- **Context:** The Azure VM (`Standard_B2as_v2`, ~$0.0492/hr, ~$35.92/month if run 24/7) runs a personal resume/portfolio project on a finite Azure for Students credit (₹8,938.35 remaining of ₹9,554.63 as of 2026-09-22, expires 2027-09-05); cost is the explicit top priority. The audience is US/EU recruiters, not IST business hours: mapped to IST, EU/US-East/US-West business hours together cover roughly 12:30pm through ~6:30am the next day, meaning there is no window that is both "IST night/sleep hours" (the operator's stated preference: "when I wake up, it should be awake") and zero recruiter-hours overlap — a real trade-off, not an engineering problem to solve around.
- **Decision:** Auto-shutdown 01:00 IST (Azure Portal's free built-in Auto-shutdown blade, no extra resource) and auto-start 07:00 IST via a new Consumption-plan Logic App `oryxenai-vm-autostart` (system-assigned managed identity, `Virtual Machine Contributor` role scoped to just this VM). ~6-hour daily shutdown window, chosen to meaningfully cut the compute cost line item (~27%) while only grazing the tail end of US-West evening browsing rather than EU or US-East prime time. See `docs/deployment/vm-cost-automation.md` for full operational detail, including why the Logic App's action is a raw ARM HTTP call instead of the native "Start virtual machine" connector (that connector's API connection would not persist in the Portal — repeatable "Failed to fetch" + vanishing workflow).
- **Rejected alternatives:** Microsoft's own "Start/Stop VMs v2" (deploys 5 Logic Apps + an Azure Functions app + Storage + Application Insights; in maintenance mode per its own docs banner; disproportionate for automating one VM). A GitHub Actions scheduled (`cron`) workflow — technically simpler and already the project's established automation pattern (self-hosted runner CD, D-110), but GitHub only fires `schedule` triggers from the repository's default branch, which is `main`, and `main` is ~632 commits behind the actual working line (`deployment`/`staging`); reconciling that was out of scope for this task. A wider/full-night shutdown window covering all of IST sleep hours — rejected because it cuts materially deeper into US-East and EU prime recruiting hours, not just sleep hours.
- **Consequence:** The VM is expected to be unreachable roughly 01:00-07:00 IST every day going forward — this is intentional, not an incident; do not "fix" it by disabling the schedule without asking the operator first. Deallocating only stops the compute billing meter; the OS disk and static public IP continue billing 24/7 regardless, so total bill savings are smaller than the ~27% compute-only figure. If editing the Logic App in the future, expect the same native-connector persistence bug in the Azure Portal and prefer the working ARM HTTP action already in place.
- **2026-09-22 addendum — "wake on click" explicitly considered and rejected:** a scale-to-near-zero design (default VM off, an always-on tiny front door wakes it on the first incoming request with a "warming up" holding page, idle-timeout auto-shutdown after inactivity) was proposed and would cut cost far more than this fixed window — but was rejected at the operator's explicit request for simplicity and reliability over further cost cuts on a resume-facing link recruiters click exactly once. The real blocker: cold start from fully deallocated is ~1.5-3 minutes (VM boot + Compose stack health), a genuine risk of looking broken to a recruiter who doesn't know to wait, on top of needing a new Azure resource and a second subdomain. Revisit only if a future operator explicitly asks for it again, ideally after checking real Cost Management trend data first (the ₹8,938-remaining-credit estimate that motivated this was extrapolated from a partial first month and needs real multi-month data to confirm before treating it as urgent).
