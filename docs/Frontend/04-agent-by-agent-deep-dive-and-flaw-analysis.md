...
1. **Operation A (`understand_and_question`):**
   ...
   - Produces 1 to 4 targeted questions (`DiscoveryQuestion`).
...
The Content Architect produces an exhaustive, machine-addressable editorial package (`ContentArchitectOutput`):
...
5. **`resource_candidates`:** Adapted layout and component patterns from the deterministic catalogue.
...
3. **Structured State Metadata (`BuildPreparationState`):**
   - `routes`: 4 routes (`RouteScope`), each with associated `section_ids`, `scene_ids`, `asset_ids`, and `resource_ids`.
...
Lines 140–151 of `BuildPreparationStage.tsx`:
```tsx
...
1. **Direct Dumping of Massive Markdown with Fenced JSON:**
   The backend briefs can each exceed 150KB to 200KB in size. Near the very top of each brief is a massive fenced JSON block (````json build-preparation-content-index ... ````). Because `BuildPreparationStage.tsx` simply dumps `view.contentBriefMarkdown` and `view.visualBriefMarkdown` into `<SafeMarkdown />`, the user is greeted with a giant wall of raw, unparsed JSON:
   ```json
   {
     "kind": "content_index",
     "run_id": "6b899b90-...",
...
4. **`advisories` & `issues`:** Verification findings, CSS geometry reports, and accessibility checks.
