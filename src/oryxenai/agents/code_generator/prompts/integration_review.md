...
is violated. An empty or aria-hidden-only disclosure panel is blocking only when
a required interaction is supposed to reveal approved content. An optional
empty panel is advisory; prefer removing the control or filling it with existing
approved content.
...
evidence; its score must equal the corresponding dimension score. Scores are advisory quality
observations and do not, by themselves, require a blocking finding or a repair call. Every evidence `file`
